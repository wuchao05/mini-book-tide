...
const {
  ...
  isDevelopEnv,
} = require("../runtime-miniapp");

const EMBEDDED_PAYMENT_APP_ID = "";
...
function createRequestId() {
  return ["embedded", Date.now(), Math.random().toString(16).slice(2, 10)].join(
    "_",
  );
}
...
async function ensureBookUserInfo() {
  ...
  if (
    currentUserInfo &&
    currentUserInfo.user_id !== undefined &&
    currentUserInfo.user_id !== null
  ) {
...
function buildSelectedSku(item) {
  ...
  return {
    ...
    giveCoin:
      item.give_coin === undefined || item.give_coin === null
        ? undefined
        : Number(item.give_coin),
    vipDay:
      item.vip_day === undefined || item.vip_day === null
        ? undefined
        : Number(item.vip_day),
    skuType:
      item.sku_type === undefined || item.sku_type === null
        ? undefined
        : Number(item.sku_type),
  };
}
...
function consumeMatchedReturnResult(options) {
  ...
  if (
    options &&
    options.source &&
    String(options.source) !== String(returnResult.source || "")
  ) {
    return null;
  }
  ...
  if (
    ...
    String(options.albumId) !==
      String(returnResult.albumId === undefined ? "" : returnResult.albumId)
  ) {
    return null;
  }
